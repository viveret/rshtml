...
pub struct AppContentBrowserMiddleware {
    file_service: Rc<dyn IAppContentProviderService>,
    next: RefCell<Option<Rc<dyn IRequestMiddlewareService>>>
}

impl AppContentBrowserMiddleware {
    ...
    pub fn new(file_service: Rc<dyn IAppContentProviderService>) -> Self {
    ...
    pub fn new_service(services: &dyn IServiceCollection) -> Vec<Box<dyn Any>> {
        vec![Box::new(Rc::new(Self::new(
            ServiceCollectionExtensions::get_required_single::<dyn IAppContentProviderService>(services)
...
impl IRequestMiddlewareService for AppContentBrowserMiddleware {
    ...
    fn handle_request(&self, response_context: &dyn IResponseContext, request_context: &dyn IRequestContext, services: &dyn IServiceCollection) -> Result<MiddlewareResult, Rc<dyn Error>> {
        ...
        let url = url.path();
        let base_path = "/appcontent-browser/";
    ...
    fn get_type_info(&self) -> Box<TypeInfo> {
        Box::new(TypeInfo::of::<AppContentBrowserMiddleware>())
    }
}