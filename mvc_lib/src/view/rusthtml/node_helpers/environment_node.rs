...
impl IHtmlNodeParsed for EnvironmentHtmlNodeParsed {
    ...
    fn on_node_parsed(&self, tag_context: Rc<dyn IHtmlTagParseContext>) -> Result<bool, RustHtmlError> {
        let html_context = tag_context.get_main_context();
        let env_name = html_context.get_environment_name();

        let mut keep_or_remove = None;

        if let Some(tokens) = tag_context.get_html_attr("include") {
            keep_or_remove = Some(process_tokens(&tokens, &env_name, true));
        }

        if keep_or_remove.is_none() {
            if let Some(tokens) = tag_context.get_html_attr("exclude") {
                keep_or_remove = Some(process_tokens(&tokens, &env_name, false));
            }
        }

        match keep_or_remove {
            Some(true) => {
                tag_context.set_only_add_inner(true);
                Ok(true)
            }
            ...
            None => Err(RustHtmlError::from_string(format!(
                "rust html tag environment expects attribute 'include' or 'exclude' to be defined (attrs: {:?})",
                tag_context.get_html_attrs()
            ))),
        }
    }
}

fn process_tokens(tokens: &[RustHtmlToken], env_name: &str, is_include: bool) -> bool {
    for token in tokens {
        if let Some(value) = process_token(env_name, is_include, token) {
            if value == true {
                return value;
            }
        }
...
fn process_token(env_name: &str, is_include: bool, token: &RustHtmlToken) -> Option<bool> {
    match token {
        RustHtmlToken::HtmlTagAttributeValue(value_string, value_literal, v_parts, rust_value) => {
            if let Some(rust_value) = rust_value {
                for v in rust_value {
                    if let RustHtmlToken::Literal(literal, string) = v {
                        let literal_as_str = unescape_literal(literal, string);
                        return Some(compare_env(env_name, &literal_as_str, is_include));
                    } else {
                        return None;
                    }
                }
            } else if let Some(v_parts) = v_parts {
                if let RustHtmlIdentAndPunctOrLiteral::IdentAndPunct(ident_and_punct) = v_parts {
                    for v in ident_and_punct {
                        if let RustHtmlIdentOrPunct::Ident(ident) = v {
                            return Some(compare_env(env_name, &ident.to_string(), is_include));
                        } else {
                            return None;
                        }
                    }
                ...
                let value_as_str = snailquote::unescape(value).expect("snailquote::unescape");
                return Some(compare_env(env_name, &value_as_str, is_include));
            } else if let Some(value_literal) = value_literal {
                return Some(compare_env(env_name, &value_literal.to_string(), is_include));
            }
        }
        RustHtmlToken::Literal(literal, string) => {
            let literal_as_str = unescape_literal(literal, string);
...
fn unescape_literal(literal: &Option<impl ToString>, string: &Option<String>) -> String {
    snailquote::unescape(&literal.as_ref().map_or_else(|| string.clone().unwrap_or_default(), |l| l.to_string()))
        .expect("snailquote::unescape")
}

fn compare_env(env_name: &str, value: &str, is_include: bool) -> bool {
    let env_name = env_name.to_lowercase();
    let value = value.to_lowercase();
    if value.contains(",") {
        let values = value.split(",");
        for value in values {
            if compare_env(&env_name, value, is_include) {
                return true;
            }
        }
        false
