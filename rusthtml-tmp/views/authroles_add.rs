...
} impl view_authroles_add
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 27 Mar 2025 02:26:43 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_authroles_add
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                m.as_ref().as_any().downcast_ref :: < AddViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < AddViewModel >
                ()).as_str()).clone()
        ...
        from(custom_html.link(url.url_action(false, Some(false), None,
        Some("index"), Some("AuthRoles"), None, None).as_str(),
        "< Back to auth roles list", None)));
        html_output.write_html_str("<h1"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(view_context.get_str("Title")));
        html_output.write_html_str("</h1>"); if let Some(validation_result) =
        & model.validation_result
        {
            let html_class = if validation_result.has_errors { "fc-error" }
            else { "fc-success" }; html_output.write_html_str("<p");
            html_output.write_html_str(" ");
            html_output.write_html_str("class");
            html_output.write_html_str("=");
