...
} impl view_authroles_index
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Wed, 26 Feb 2025 01:32:42 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_authroles_index
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                authroles :: index :: IndexViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                authroles :: index :: IndexViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: authroles ::
        index :: IndexViewModel > :: new(view_context, services); let render =
