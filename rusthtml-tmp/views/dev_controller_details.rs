...
} impl view_dev_controller_details
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 27 Mar 2025 02:27:03 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_controller_details
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: controllers :: ControllerDetailsViewModel >
                ().expect(format!
        ...
        controllers :: ControllerDetailsViewModel > ::
        ...
        } let route_path = model.name; let title = format!
        ("Controller details of {}", route_path);
        view_context.insert_str("Title", title.clone()); let actions =
        model.actions; let controller_features = model.features; let
        controller_attributes = model.attributes; let controller_properties =
        model.properties; let controller_methods = model.methods;
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("controllers"), Some("Dev"), None, None).as_str(),
        "< Back to controllers list", None)));
        ...
        html_output.write_html(HtmlString ::
        from(format!
        ("Controller Properties ({}):", controller_properties.len())));
        html_output.write_html_str("</h3>");
        html_output.write_html_str("<ol"); html_output.write_html_str(">");
        ...
        html_output.write_html_str("<h3"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(format! ("Actions ({}):", actions.len())));
        html_output.write_html_str("</h3>");
        html_output.write_html_str("<ol"); html_output.write_html_str(">");
