...
} impl view_dev_controllers
{
    pub fn new() -> Self
    {
        ...
        {
            model_type_name :
            "crate::view_models::dev::controllers::ControllersViewModel",
            ViewPath : file! (), raw : "", when_compiled : DateTime ::
            parse_from_rfc2822("Wed, 26 Feb 2025 01:32:52 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_controllers
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: controllers :: ControllersViewModel >
                ().expect(format!
        ...
        controllers :: ControllersViewModel > :: new(view_context, services);
        ...
        html_output.write_html(HtmlString ::
        from(format!
        ("In total there are {} controllers:", model.controllers.len())));
        html_output.write_html_str("</p>"); html_output.write_html_str("<ul");
        html_output.write_html_str(">"); for controller in
        model.controllers.iter()
        {
            let link_text = & controller.name; let link_href =
            url.url_action(false, Some(false), None,
            Some("controller_details"), Some("Dev"), None,
            Some(& RouteValuesBuilder :: build_area(& controller.name)));
