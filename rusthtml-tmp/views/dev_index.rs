...
ServiceCollectionExtensions; pub struct view_dev_index
{
...
} impl view_dev_index
{
    pub fn new() -> Self
    {
        ...
        {
            model_type_name :
            "crate::view_models::dev::index::IndexViewModel", ViewPath : file!
            (), raw : "", when_compiled : DateTime ::
            parse_from_rfc2822("Tue, 01 Oct 2024 23:58:28 +0000").expect("could not parse when compiled").into(),
        }
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: index :: IndexViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                dev :: index :: IndexViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: dev :: index ::
        IndexViewModel > :: new(view_context, services); let render =
        ...
        Some("Dev"), None, None).as_str(), "Log", None)));
        html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("perf_log"), Some("Dev"), None, None).as_str(),
        "Performance Log", None))); html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("controllers"), Some("Dev"), None, None).as_str(), "Controllers",
        None))); html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("routes"), Some("Dev"), None, None).as_str(), "Routes", None)));
        html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None, Some("views"),
        Some("Dev"), None, None).as_str(), "Compiled views", None)));
        html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("sys_info"), Some("Dev"), None, None).as_str(), "Sys Info",
        None))); html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None, Some("index"),
        Some("AuthRoles"), None, None).as_str(), "Auth Roles", None)));
        html_output.write_html_str("</li>");
        html_output.write_html_str("<li"); html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None, Some("error"),
        Some("Dev"), None, None).as_str(), "Return Error", None)));
        html_output.write_html_str("</li>");
