...
} impl view_dev_log
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 13 Mar 2025 16:48:41 +0000").expect("could not parse when compiled").into(),
        }
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: log :: LogViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                dev :: log :: LogViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: dev :: log ::
        LogViewModel > :: new(view_context, services); let render =
        ...
        html_output.write_html_str("</h1>"); if model.supports_read
        {
            html_output.write_html_str("<p"); html_output.write_html_str(">");
            html_output.write_html(HtmlString ::
            from(format! ("There are {} log entries", model.logs.len())));
