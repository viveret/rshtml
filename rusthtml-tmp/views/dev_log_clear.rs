...
} impl view_dev_log_clear
{
    pub fn new() -> Self
    {
        ...
        {
            model_type_name :
            "crate::view_models::dev::log_clear::LogClearViewModel", ViewPath
            : file! (), raw : "", when_compiled : DateTime ::
            parse_from_rfc2822("Thu, 27 Mar 2025 02:27:18 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_log_clear
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: log_clear :: LogClearViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                dev :: log_clear :: LogClearViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: dev ::
        log_clear :: LogClearViewModel > :: new(view_context, services); let
        render = RenderHelpers :: new(view_context, services); let url =
        ...
        html_output.write_html_str("</h1>"); html_output.write_html_str("<b");
        html_output.write_html_str(">");
        html_output.write_html_str("todo: Not implemented yet");
        html_output.write_html_str("</b>"); Ok(html_output.collect_html())
    }
}