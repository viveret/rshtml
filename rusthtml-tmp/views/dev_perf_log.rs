...
} impl view_dev_perf_log
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 27 Mar 2025 02:27:23 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_perf_log
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: perf_log :: PerfLogViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                dev :: perf_log :: PerfLogViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: dev :: perf_log
        :: PerfLogViewModel > :: new(view_context, services); let render =
        ...
        Some("Dev"), None, None).as_str(), "< Back to dev routes", None)));
