...
} impl view_dev_route_details
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 27 Mar 2025 02:27:43 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_route_details
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: route_details :: RouteDetailsViewModel >
                ().expect(format!
        ...
        route_details :: RouteDetailsViewModel > ::
        ...
        } let route_path = model.path; let title = format!
        ("Route details of {}", route_path);
        view_context.insert_str("Title", title.clone()); let action_features =
        model.features; let controller_features = model.controller_features;
        html_output.write_html(HtmlString ::
        from(html.link(url.url_action(false, Some(false), None,
        Some("routes"), Some("Dev"), None, None).as_str(),
        "< Back to routes list", None))); html_output.write_html_str("<h1");
