...
} impl view_dev_routes
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 13 Mar 2025 16:49:11 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_routes
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: routes :: RoutesViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                dev :: routes :: RoutesViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: dev :: routes
        :: RoutesViewModel > :: new(view_context, services); let render =
        ...
        html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(format! ("In total there are {} routes:", model.routes.len())));
