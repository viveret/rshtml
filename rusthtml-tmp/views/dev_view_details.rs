...
} impl view_dev_view_details
{
    pub fn new() -> Self
    {
        ...
        {
            model_type_name :
            "crate::view_models::dev::view_details::ViewDetailsViewModel",
            ViewPath : file! (), raw : "", when_compiled : DateTime ::
            parse_from_rfc2822("Thu, 27 Mar 2025 02:27:33 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_dev_view_details
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                dev :: view_details :: ViewDetailsViewModel >
                ().expect(format!
        ...
        }; let html = HtmlHelpers :: < crate :: view_models :: dev ::
        view_details :: ViewDetailsViewModel > :: new(view_context, services);
        let render = RenderHelpers :: new(view_context, services); let url =
        UrlHelpers :: new(view_context, services); let html_output =
        HtmlBuffer :: new(); match
        ...
        view_context.insert_str("Title", title.clone()); let raw = model.raw;
        let statements = raw.split([';', '{', '}',]); let model_type_name =
        match model.model_type_name
        {
        ...
        Some("Dev"), None, None).as_str(), "< Back to views list", None)));
        html_output.write_html_str("<h1"); html_output.write_html_str(">");
        html_output.write_html(HtmlString :: from(title));
        ...
        for s in statements
        {
            ...
            html_output.write_html(HtmlString :: from(s));
            html_output.write_html_str("</li>");
