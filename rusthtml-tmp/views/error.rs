...
ServiceCollectionExtensions; pub struct view_error
{
...
} impl view_error
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 27 Mar 2025 02:28:18 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_error
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                m.as_ref().as_any().downcast_ref :: < mvc_lib :: error ::
                error_viewmodel_service :: BasicErrorViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < mvc_lib :: error ::
                error_viewmodel_service :: BasicErrorViewModel >
                ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < mvc_lib :: error ::
        error_viewmodel_service :: BasicErrorViewModel > ::
        ...
        html_output.write_html(HtmlString ::
        from(format! ("{}", model.error)));
        html_output.write_html_str("</h3>"); html_output.write_html_str("<p");
        html_output.write_html_str(">");
        html_output.write_html(HtmlString ::
        from(format! ("Source: {:?}", model.error.source())));
        html_output.write_html_str("</p>"); Ok(html_output.collect_html())
    }
}