...
} impl view_learn_details
{
    pub fn new() -> Self
    {
        Self
        {
            ...
            parse_from_rfc2822("Thu, 13 Mar 2025 16:49:41 +0000").expect("could not parse when compiled").into(),
        }
...
} impl IView for view_learn_details
{
    ...
    IServiceCollection) -> Result < HtmlString, RustHtmlError >
    {
        let vm = view_context.get_viewmodel(); let model = match vm
        {
            Some(m) =>
            {
                ...
                learn :: DetailsViewModel >
                ().expect(format!
                ("could not downcast model from Rc<dyn IModel>({:?}) to {:?}",
                m.get_type_info(), TypeInfo :: of :: < crate :: view_models ::
                learn :: DetailsViewModel > ()).as_str()).clone()
            }, None => panic! ("No model set")
        }; let html = HtmlHelpers :: < crate :: view_models :: learn ::
        DetailsViewModel > :: new(view_context, services); let render =
        ...
        "learn/_view_start.rs".to_string(), view_context.get_viewmodel(),
